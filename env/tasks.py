...
def _base_menu() -> list[MenuItem]:
    return [
        MenuItem(name="Butter Chicken", price=350, cost=120, prep_time_minutes=20, category="main",
                 ingredients={"chicken": 0.5, "spices": 0.1, "butter": 0.1}),
        MenuItem(name="Paneer Tikka", price=280, cost=90, prep_time_minutes=15, category="main",
                 ingredients={"paneer": 0.4, "spices": 0.1}),
        MenuItem(name="Dal Tadka", price=180, cost=40, prep_time_minutes=12, category="main",
                 ingredients={"lentils": 0.3, "spices": 0.05, "butter": 0.05}),
        MenuItem(name="Naan", price=60, cost=15, prep_time_minutes=5, category="appetizer",
                 ingredients={"flour": 0.2, "butter": 0.02}),
        MenuItem(name="Gulab Jamun", price=120, cost=35, prep_time_minutes=8, category="dessert",
                 ingredients={"flour": 0.1, "sugar": 0.15}),
        MenuItem(name="Mango Lassi", price=100, cost=25, prep_time_minutes=3, category="drink",
...
def task_crisis_shift() -> dict:
    ...
    for item in inventory:
        item.quantity = round(item.quantity * 0.4, 1)
    return {
...
def get_task(task_id: str) -> dict:
    if task_id not in TASK_REGISTRY:
        raise KeyError(f"Unknown task '{task_id}'. Available: {list(TASK_REGISTRY.keys())}")
